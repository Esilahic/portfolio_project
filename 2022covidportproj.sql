...
SELECT 
    ...
    population
FROM
    portfolio_project_covid.covid_deaths_2022
ORDER BY 1 , 2;
...
FROM portfolio_project_covid.covid_deaths_2022 dea
Join portfolio_project_covid.covid_vacs_2022 vac
...
-- TEMP TABLE PRACTICE
CREATE TABLE portfolio_project_covid.PercentPopulationVaccinated2022 (
...
Insert into portfolio_project_covid.PercentPopulationVaccinated2022
Select dea.continent, dea.location, dea.date, dea.population, vac.new_vaccinations, SUM(cast(vac.new_vaccinations as signed)) OVER (Partition by dea.location Order by dea.location, dea.date) as RollingPeopleVaccinated
FROM portfolio_project_covid.covid_deaths_2022 dea
Join portfolio_project_covid.covid_vacs_2022 vac
	On dea.location = vac.location
...
FROM
    portfolio_project_covid.PercentPopulationVaccinated2022;

-- CREATE VIEW FOR LATER
CREATE VIEW portfolio_project_covid.PercentPopulationVaccinated2022VIEW as 
Select dea.continent, dea.location, dea.date, dea.population, vac.new_vaccinations, SUM(cast(vac.new_vaccinations as signed)) OVER (Partition by dea.location Order by dea.location, dea.date) as RollingPeopleVaccinated
FROM portfolio_project_covid.covid_deaths_2022 dea
Join portfolio_project_covid.covid_vacs_2022 vac
	On dea.location = vac.location
...
CREATE VIEW portfolio_project_covid.GlobalNumbers2022VIEW AS
    SELECT 
        SUM(new_cases) AS total_cases,
        SUM(CAST(new_deaths AS SIGNED)) AS total_deaths,
        SUM(CAST(new_deaths AS SIGNED)) / SUM(New_Cases) * 100 AS DeathPercentage
...
CREATE VIEW portfolio_project_covid.HighestDeaths2022VIEW AS
    SELECT 
        location,
        MAX(CAST(Total_deaths AS SIGNED)) AS TotalDeathCount
    FROM
...
CREATE VIEW portfolio_project_covid.HighestDeathsContinent2022VIEW AS
    SELECT 
        continent,
...
CREATE VIEW portfolio_project_covid.Infections2022VIEW AS
    SELECT 
        Location,
        Population,
...
CREATE VIEW portfolio_project_covid.HighestInfections2022VIEW AS
    SELECT 
        Location,
        Population,
        date,
